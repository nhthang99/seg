...
class BestSaver(ignite.handlers.ModelCheckpoint, Module):
    '''
        ...
        Parameters:
            dirname (str): directory path. A subfolder will be created in the directory.
                           Your objects will be saved in this subfolder.
            score_name (str): name of a metric attached to the engine what defines how good objects are.
            mode (str): one of min, max. In min mode, the least score objects will be saved.
                        In max mode, the best score objects will be saved.
        See Ignite ModelCheckpoint for more details about other parameters.
    '''

    def __init__(self, dirname, score_name, evaluator_name, experiment_name=None, mode='max',
                 n_saved=1, atomic=True, require_empty=True, create_dir=True):
        if mode not in ['min', 'max']:
            raise ValueError(f'mode must be min or max. mode value found is {mode}')

        if experiment_name is not None:
            dirname = os.path.join(dirname, experiment_name, datetime.now().strftime('%y%m%d%H%M'))
        else:  
            dirname = os.path.join(dirname, datetime.now().strftime('%y%m%d%H%M'))

        best_score = lambda e: e.state.metrics[score_name] if mode == 'max' else - e.state.metrics[score_name]
        super(BestSaver, self).__init__(dirname, 'best', score_function=best_score,
                                        score_name=score_name, n_saved=n_saved, atomic=atomic,
                                        require_empty=require_empty, create_dir=create_dir,
                                        global_step_transform=lambda e, event: self.frame['engine'].engine.state.epoch)
...
class BackupSaver(ignite.handlers.ModelCheckpoint, Module):
    ...
    def __init__(self, modules, dirname, save_interval, experiment_name=None, n_saved=1,
    ...
    def init(self):
        ...
        self.frame['engine'].engine.add_event_handler(Events.EPOCH_COMPLETED(every=self.save_interval),
                                                      self._correct_checpoint)

    def state_dict(self):
        return {'n_saved': self.n_saved,
                'saved': [(priority, filename) for priority, filename in self._saved],
                }
    ...
    def _correct_checpoint(self, engine):
        ...
        checkpoint_path = os.path.join(self.frame['backup_saver'].save_handler.dirname,
                                       self.frame['backup_saver']._saved[-1].filename)
        checkpoint = torch.load(checkpoint_path)
        checkpoint['backup_saver']['saved'] = [(priority, filename)
                                               for priority, filename in self.frame['backup_saver']._saved]
